...
/**
 * Build the full prompt to send to DALL-E 3.
 */
export function generateSpritePrompt(styleId, actionId, referenceToken = 'REF_CHAR', characterDescription = '', frameIndex = 0, isContinuation = false) {
  ...
  // Use character description or default
  ...
  // For original style, we'll focus on maintaining the character description exactly
  if (style.isReferenceOnly) {
    const prompt = [
      `Generate a video game sprite with transparent background.`,
